 ...
 * @param {Property|MethodDefinition|ClassProperty|TSAbstractClassProperty|TSPropertySignature|TSAbstractMethodDefinition} node The property node to collect prefix tokens.
 * @returns {Token[]}
 */
/**
...
function defineVisitor({
  ...
  return {
    ...
    TSTypeAliasDeclaration(node) {
      processNodeList(
        [node.id, node.typeAnnotation, node.typeParameters || null],
        tokenStore.getFirstToken(node),
        null,
        1
      )
    ...
    TSDeclareFunction(node) {
      ...
      while (
        token != null &&
        token.range[0] < (starToken || idToken).range[0]
      ) {
        prefixTokens.push(token)
        token = tokenStore.getTokenAfter(token)
      }
    ...
    TSEnumDeclaration() {
      console.log('not impl TSEnumDeclaration')
    },
    TSEnumMember() {
      console.log('not impl TSEnumMember')
    },
    TSExportAssignment() {
      console.log('not impl TSExportAssignment')
    },
    TSExternalModuleReference() {
      console.log('not impl TSExternalModuleReference')
    },
    TSImportType() {
      console.log('not impl TSImportType')
    },
    TSInferType() {
      console.log('not impl TSInferType')
    },
    TSLiteralType() {
      console.log('not impl TSLiteralType')
    },
    TSIndexedAccessType() {
      console.log('not impl TSIndexedAccessType')
    },
    TSImportEqualsDeclaration() {
      console.log('not impl TSImportEqualsDeclaration')
    },
    TSFunctionType() {
      console.log('not impl TSFunctionType')
    },
    TSMethodSignature() {
